...
namespace ego_planner
{
  ...
  void BsplineOptimizer::setParam(ros::NodeHandle &nh)
  {
    ...
    nh.param("optimization/dist0", dist0_, -1.0); //安全距离初始为0.5m
  ...
  /**
   * @brief 初始化控制点并分段以避开障碍物。
   *
   * 该函数使用A*搜索算法在障碍物中寻找无碰撞路径，并根据路径调整控制点。
   * 它将初始控制点分段，并计算每段的边界以确保路径的可行性。
   *
   * @param init_points 初始控制点矩阵。
   * @param flag_first_init 是否为第一次初始化控制点，默认为`true`。
   *
   * @return 返回一个包含控制点段的索引对的向量。
   *         - 每个索引对表示一段轨迹的起始和结束控制点。
   *         - 如果所有控制点都在障碍物外，则返回空向量。
   */
    ...
    bool flag_first_init /*= true*/)
  {
    ...
    if (flag_first_init)
    {
      cps_.clearance = dist0_;  // 设置初始安全距离0.5m
      cps_.resize(init_points.cols());  // 调整容器大小与控制点数量一致
      cps_.points = init_points;  // 深拷贝控制点坐标
    }

                                                  /* -----根据障碍物信息对初始轨迹进行分段----- */

    constexpr int ENOUGH_INTERVAL = 2;
    // 步长 ≈ 地图分辨率 / (控制点平均间距 × 1.5)，保证采样点能检测到狭窄障碍物
    double step_size = grid_map_->getResolution() / 
      ((init_points.col(0) - init_points.rightCols(1)).norm() / //首尾控制点的欧氏距离
      (init_points.cols() - 1)) /   // 平均每段控制点间距
      1.5;  // 确保采样密度高于地图分辨率
    // 记录进入/离开障碍物时的控制点索引
    ...
    // 标记是否检测到进入/离开障碍物的起始点/终止点, 临时标记可能结束点
    bool flag_got_start = false, flag_got_end = false, flag_got_end_maybe = false;
    // i_end:B样条轨迹优化的终止控制点索引=优化时实际使用的最后一个有效控制点的索引  计算方式不用管
    int i_end = (int)init_points.cols() - order_ - ((int)init_points.cols() - 2 * order_) / 3; 

    // 从第三个控制点开始检查两个控制点之间是否在障碍物里面，如果在障碍物里面record segment_ids
    // why i = order_  answer: B样条的前 order_ 个控制点仅用于定义初始状态，不参与优化（保持轨迹起点固定）
    for (int i = order_; i <= i_end; ++i)
    {
      // 相邻控制点i-1 和 i 间插值 判断是否在障碍物里
      for (double a = 1.0; a > 0.0; a -= step_size)
      {
        ...
        if (occ && !last_occ)
        {
          // 连续 ENOUGH_INTERVAL 次检测到占用 or 起点附近就是障碍物 --> 记录
          if (same_occ_state_times > ENOUGH_INTERVAL || i == order_)
          {
            in_id = i - 1;           // 记录进入点索引
            flag_got_start = true;   // 标记已找到起点
          }
        ...
        {
          out_id = i;                 // 记录离开点索引
          flag_got_end_maybe = true;  // 标记可能结束
          same_occ_state_times = 0;
        }
        // 当前状态与上一状态相同
        else
        ...
        if (flag_got_start && flag_got_end)
        {
          ...
          flag_got_start = flag_got_end = false;  // 重置标志
        }
      }
    }
    ...
    if (segment_ids.size() == 0)
    {
      ...
      return blank_ret;
                                                  ...
                                                  /* -----使用A*搜索算法在障碍物中寻找无碰撞路径------ */
    // 存储所有A*搜索得到的路径
    vector<vector<Eigen::Vector3d>> a_star_pathes;  
    // 遍历 segment_ids 中的每个区间段,每个区间进行碰撞检测并生成无碰撞路径
    for (size_t i = 0; i < segment_ids.size(); ++i)
    {
      // 提取当前区间的起止坐标点
      ...
      else
      {
        ...
        return blank_ret;
                                                  ...
                                                  /* -----计算分段优化边界bounds及控制点数量优化----- */
    int id_low_bound, id_up_bound;
    // pair类型的动态数组
    vector<std::pair<int, int>> bounds(segment_ids.size());
    // 为B样条曲线的分段控制点计算边界索引,通过在两段之间插入缓冲边界（取相邻段端点的中间值），确保：修改第i段的控制点时，不会影响第i+1段的曲线形状
    for (size_t i = 0; i < segment_ids.size(); i++)
    {
      // 第一段
      if (i == 0) 
      ...
      // 中间段，最低限制是当前segment_ids对，初始控制点和上一segment_ids对目标控制点
      else
      {
        id_low_bound = (int)(((segment_ids[i].first + segment_ids[i - 1].second) + 1.0f) / 2); 
    ...
    // minimum_points:当前分段至少需要的控制点数; num_points:实际分配给当前分段的控制点数
    int minimum_points = round(init_points.cols() * MINIMUM_PERCENT), num_points;

    // 调整 B 样条曲线的分段控制点范围，确保每个分段至少包含 minimum_points 个控制点，同时不超过 bounds 的安全边界
    for (size_t i = 0; i < segment_ids.size(); i++)
    {
      num_points = segment_ids[i].second - segment_ids[i].first + 1;
      cout << "i segment = " << i << " first = " << segment_ids[i].first << " second = " << segment_ids[i].second << endl;
      // 当前分段的控制点数不足
      if (num_points < minimum_points)
      {
        double add_points_each_side = (int)(((minimum_points - num_points) + 1.0f) / 2);
        // 尝试向左扩展 add_points_each_side 个点，但不能超过 bounds[i].first（安全下界
        adjusted_segment_ids[i].first = segment_ids[i].first - add_points_each_side >= bounds[i].first ? 
                                        ...
                                        bounds[i].first;
        // 尝试向右扩展 add_points_each_side 个点，但不能超过 bounds[i].second（安全上界
      ...
      cout << "final:" << "i adjusted_segment= " << i << " first = " << adjusted_segment_ids[i].first << " second = " << adjusted_segment_ids[i].second << endl;
    }
    ...
    for (size_t i = 1; i < adjusted_segment_ids.size(); i++) 
    {
      if (adjusted_segment_ids[i - 1].second >= adjusted_segment_ids[i].first)
      {
        ...
        // 无需检查越界bounds，因为 adjusted_segment_ids 已在 bounds 内
      }
    }
                                                    /* -----计算分段优化边界bounds及控制点数量优化----- */
    ...
    for (size_t i = 0; i < segment_ids.size(); i++)
    {
      // step 1 当前分段的控制点标记为无需被调整
      for (int j = adjusted_segment_ids[i].first; j <= adjusted_segment_ids[i].second; ++j)
        cps_.flag_temp[j] = false;

      // step 2 检测控制点与障碍物的交点，并调整控制点的位置以避免碰撞
      int got_intersection_id = -1;
      // 遍历当前分段的内部控制点（跳过首尾点以保持连续性）
      for (int j = segment_ids[i].first + 1; j < segment_ids[i].second; ++j)
      {
        // 计算控制点切线方向，在B样条曲线中，控制点的差分可以近似表示曲线的切线方向
        ...
        while (Astar_id >= 0 && Astar_id < (int)a_star_pathes[i].size())
        {
          ...
          // 如果检测到符号变化: A* 路径穿过了切线方向的垂直平面（且非零），通过线性插值法计算计算该交点坐标
          if (val * last_val <= 0 && (abs(val) > 0 || abs(last_val) > 0)) 
          {
            intersection_point = a_star_pathes[i][Astar_id] +
                ((a_star_pathes[i][Astar_id] - a_star_pathes[i][last_Astar_id]) *
                 ...
                 ctrl_pts_law.dot(a_star_pathes[i][Astar_id] - a_star_pathes[i][last_Astar_id])));
        ...
        if (got_intersection_id >= 0)
        {
          ...
          if (length > 1e-5)
          {
            ...
            // 从交点位置一步步往回走，检查哪些位置是安全的
            for (double a = length; a >= 0.0; a -= grid_map_->getResolution())
            {
              // 检查线性插值创建的位置: (a / length) * 障碍物位置 + (1 - a / length) * 控制点位置是否有障碍物
              occ = grid_map_->getInflateOccupancy((a / length) * intersection_point + (1 - a / length) * init_points.col(j));

              // 检测到障碍物（occ）或达到最小步长:已经搜索完所有可能位置
      ...
      // 特殊情况：线段长度过短。在这种情况下，控制点可能会超出 A* 路径范围，从而导致梯度方向相反。
      ...
      // step 3 通过前、后向传播实现无需被调整的控制点复制改点的安全位置和方向，从而保证路径的连续性和平滑性
      if (got_intersection_id >= 0)
      {
...
/* 控制点距离最近障碍物的距离cost */
  ...
  {
    cost = 0.0; //初始化cost
    int end_idx = q.cols() - order_;//只对N-3个控制点做处理
    double demarcation = cps_.clearance;//安全系数，对应论文里面的sf
    /* a = 3*sf    b = -3* sf^2    c = sf^sf^sf */
  ...
  void BsplineOptimizer::calcFitnessCost(const Eigen::MatrixXd &q, double &cost, Eigen::MatrixXd &gradient)
  {
    ...
    for (auto i = order_ - 1; i < end_idx + 1; ++i)
    {
      ...
      */
      ...
      /* 求梯度，用cost对影响这一cost的控制点求导，然后在循环中对cost及各控制点梯度进行累加*/
  ...
  /* 这一块和论文是有区别的 论文里面的平滑项惩罚cost是jerk和acc的和，这里面是只用了jerk*/
                                            ...
                                            Eigen::MatrixXd &gradient, bool falg_use_jerk /* = true*/)
  {
    ...
    /* 默认使用jerk 平顺项进行处理，可利用均匀B样条微分公式求出*/
    if (falg_use_jerk)
    {
      Eigen::Vector3d jerk, temp_j;
      /* 去掉起点终点轨迹 N-order_控制点个数 */
      for (int i = 0; i < q.cols() - 3; i++)
      {
        /* evaluate jerk */
        /*   对p(s(u))求三阶导得到j
          *  j=(1/6*s(u)*s(u)*s(u)*[-1,3,-3,1]*qmT)'''=[-1,3,-3,1]*qmT
        **/
        jerk = q.col(i + 3) - 3 * q.col(i + 2) + 3 * q.col(i + 1) - q.col(i);

        /* 向量的平方范数由squaredNorm()获得，等价于向量对自身做点积*/
        cost += jerk.squaredNorm();
        temp_j = 2.0 * jerk;
        /* jerk gradient */
        /* 求梯度，用cost对影响这一cost的控制点求导，然后在循环中对cost及各控制点梯度进行累加
        *d(cost)/dq[i + 1] = d(cost)/d(jerk)*d(jerk)/dq[i + 1] = 2* jerk*3=6*jerk= 3*temp_j
        *d(cost)/dq[i] = d(cost)/d(jerk)*d(jerk)/dq[i] = 2* jerk*(-1)
        *d(cost)/dq[i+3] = d(cost)/d(jerk)*d(jerk)/dq[i+3] = 2* jerk*1
        *d(cost)/dq[i+2] = d(cost)/d(jerk)*d(jerk)/dq[i+2] = 2* jerk*(-3)
        */
...
/*动力学合理性检查，限制速度和加速度*/
                                             ...
                                             Eigen::MatrixXd &gradient)
  {
    ...
    ts = bspline_interval_; //控制点中的节点向量
    ts_inv2 = 1 / ts / ts;
    ...
    for (int i = 0; i < q.cols() - 1; i++)
    {
      ...
      for (int j = 0; j < 3; j++)
      {
        if (vi(j) > max_vel_)
        {
          ...
          // cout << vi(j) << endl;
          /* cost += (两个控制点之点的速度-最大速度)^2，不过为什么多了ts_inv2？*/
          cost += pow(vi(j) - max_vel_, 2) * ts_inv2; // multiply ts_inv3 to make vel and acc has similar magnitude
           
        /*求梯度，用cost对影响这一cost的控制点求导，然后在循环中对cost及各控制点梯度进行累加
        *dcost/dq.col(i + 1) = dcost/dvi(j)*dvi(j)/dq.col(i + 1) = 2 * (vi(j) - max_vel_)/ts*ts_inv2
        *dcost/dq.col(i) = dcost/dvi(j)*dvi(j)/dq.col(i]) = -2 * (vi(j) - max_vel_)/ts*ts_inv2
        */
    ...
    for (int i = 0; i < q.cols() - 2; i++)
    {
      ...
      for (int j = 0; j < 3; j++)
      {
        if (ai(j) > max_acc_)
        {
         ...
         /*求梯度，用cost对影响这一cost的控制点求导，然后在循环中对cost及各控制点梯度进行累加
        *dcost/dq.col(i + 1) = dcost/dai(j)*dai(j)/dq.col(i + 1) = -4 * (ai(j) - max_acc_)*ts_inv2
        *dcost/dq.col(i) = dcost/dai(j)*dai(j)/dq.col(i]) = 2 * (ai(j) - max_acc_)*ts_inv2
        *dcost/dq.col(i+2) = dcost/dai(j)*dai(j)/dq.col(i]) = 2 * (ai(j) - max_acc_)*ts_inv2
        */
  ...
  {
    setBsplineInterval(ts);//节点向量

    cps_ = control_points;//控制点

    bool flag_success = rebound_optimize(final_cost);

    optimal_points = cps_.points;//得到优化的控制点
  ...
  /**
   * @brief 使用L-BFGS优化算法对B样条轨迹进行优化（避障+平滑性优化）
   * 
   * @param[out] final_cost 优化后的最终代价（通过引用返回）
   * @return bool 优化是否成功（true=成功，false=失败）
   * 
   * @note 主要流程：
   * 1. 初始化优化变量和参数。
   * 2. 调用L-BFGS优化器进行非线性优化。
   * 3. 分阶段检查碰撞和群体距离约束。
   * 4. 根据检查结果决定是否重启优化或终止。
   */
  ...
  {
    iter_num_ = 0;
    int start_id = order_;                     // 起始控制点索引
    // int end_id = this->cps_.size - order_;  // Fixed end
    int end_id = this->cps_.size;              // 终止控制点索引
    variable_num_ = 3 * (end_id - start_id);   // 优化变量总数（每个点xyz三自由度）
    ...
    do
    {
      ...
      double q[variable_num_];                                                      // 优化变量数组，存储控制点的 (x,y,z) 坐标
      memcpy(q, cps_.points.data() + 3 * start_id, variable_num_ * sizeof(q[0]));   // 从 cps_.points 中提取可优化控制点

      // 设置拟牛顿法(L-BFGS)优化器的超参数
      ...
      int result = lbfgs::lbfgs_optimize(
          variable_num_, q, &final_cost,          
          BsplineOptimizer::costFunctionRebound,  // 代价函数
          NULL, 
      ...
      // 记录优化耗时
      t2 = ros::Time::now();
      double time_ms = (t2 - t1).toSec() * 1000;        // 单次优化耗时（毫秒）
      double total_time_ms = (t2 - t0).toSec() * 1000;  // 总耗时（毫秒）

                                                              /* ------ 优化后检查 ------ */
      // 检查L-BFGS结果(L-BFGS收敛或达到最大迭代次数时条件成立）                                                      
          ...
          result == lbfgs::LBFGS_STOP)
      {
        ...
        double t_step = (tmp - tm) / (                                        // 根据轨迹长度和地图分辨率 动态调整碰撞检查的时间步长
            (traj.evaluateDeBoorT(tmp) - traj.evaluateDeBoorT(tm)).norm()     // 轨迹起点到终点的位移向量
            / grid_map_->getResolution()
          );

        // 执行轨迹时会动态调整后续路径，因此优先确保当前最接近的部分（前2/3）的安全性
        for (double t = tm; t < tmp * 2 / 3; t += t_step) 
        {
          ...
          if (flag_occ)
          {
            ...
            // 前3个控制点对应的时间段发生碰撞, 机器人无法从障碍物内部启动（路径不可行）
...
// #define USE_SECOND_CLEARENCE_CHECK
        ...
        for (Eigen::Index i = 0; i < cps_.points.cols(); i++)
        {
          ...
          check_pts(0, i) += offset_xy(0);n
          check_pts(1, i) += offset_xy(1);
          check_pts(2, i) += offset_xy(2);/* ------ 优化后检查 ------ */
        }
